...
def preprocess(df):
    ...
    # ✅ FIX (ALIGN PROPERLY — SAME INDENT LEVEL)
    threshold = df["risk_label"].median()
    df["risk_label"] = (df["risk_label"] > threshold).astype(int)
...
def main():
    ...
    # Final check
    assert len(FEATURE_COLS) == X.shape[1], "Feature mismatch error"
